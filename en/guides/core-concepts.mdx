...
Resources like people, companies, deals, and products support **soft delete**. When deleted via the API, the record is marked as deleted internally but not permanently removed from the database.

From your integration's perspective, the behavior is transparent:

- `DELETE /v1/tasks/:id` returns `204` and the record becomes inaccessible
- `GET /v1/tasks/:id` for a deleted record returns `404`
