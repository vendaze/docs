...
<Note>
  You never send the `workspace_id` in requests. It is embedded in the `access_token` at
  authorization time and resolved automatically by the API on every call.
</Note>
