...
<Accordion title="forbidden (403)">
  The token is valid and has the right scope, but the authenticated workspace is not allowed to
  access this specific resource. This can happen when trying to access a record that belongs to a
  different workspace.
</Accordion>
