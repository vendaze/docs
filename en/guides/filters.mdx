...
| `email`          | string   | contains (ilike) | Filter by email address. Partial match across all emails on the contact. |
| `phone`          | string   | contains (ilike) | Filter by phone number. Partial match across all phones on the contact.  |
| `company_id`     | UUID     | exact            | Returns only people linked to the given company.                       |
| `has_email`      | boolean  | exact            | `true` to return only people with an email, `false` for people without.|
| `has_phone`      | boolean  | exact            | `true` to return only people with a phone, `false` for people without. |
