...
| `avatar_url`    | string   | No       | HTTPS URL of your app logo. Vendaze fetches the image, validates it, converts it to JPEG (max 400x400px), and stores it. Accepted formats: JPEG, PNG, WebP, GIF, BMP, TIFF. SVG is not accepted. Max 2MB. |
...
When `avatar_url` is provided, Vendaze fetches the image from your server, validates the file format using magic bytes (not the declared Content-Type), converts it to JPEG (max 400x400px), and stores it on Vendaze infrastructure. The `avatar_url` in the response is always a `https://storage.vendaze.com` URL.
