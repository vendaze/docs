...
description: 'Entenda os fundamentos da API do Vendaze antes de começar a integrar.'
icon: 'book'
---
...
<Note>
  Você nunca envia o `workspace_id` nas requisições. Ele é embutido no `access_token` no momento da
  autorização e resolvido automaticamente pela API em cada chamada.
</Note>
...
1. Registra-se no Vendaze para receber `client_id` e `client_secret`
