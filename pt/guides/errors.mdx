...
| `invalid_client`      | 401  | `client_id` ou `client_secret` inválido nos endpoints OAuth.                                  |
| `invalid_grant`       | 400  | Código de autorização inválido, expirado ou já utilizado. Refresh token inválido ou expirado. |
...
| `conflict`            | 409  | Conflito com dados existentes. Veja `message` para detalhes.                                  |
...
async function apiCall(url, options) {
  ...
  if (!res.ok) {
    ...
    switch (error.code) {
      ...
      case 'invalid_grant':
        // Código de autorização expirado/usado, ou refresh token inválido.
        // Redirecione o usuário pelo fluxo OAuth completo novamente.
        throw new Error('Authorization expired. User must re-authorize.');

      case 'invalid_client':
        // client_id ou client_secret incorreto. Verifique suas credenciais.
