...
| `email`          | string   | contém (ilike) | Filtra por endereço de email. Match parcial em todos os emails do contato.   |
| `phone`          | string   | contém (ilike) | Filtra por número de telefone. Match parcial em todos os telefones do contato.|
| `company_id`     | UUID     | exato          | Retorna apenas pessoas vinculadas à empresa informada.                       |
| `has_email`      | boolean  | exato          | `true` para retornar apenas pessoas com email, `false` para pessoas sem email.|
| `has_phone`      | boolean  | exato          | `true` para retornar apenas pessoas com telefone, `false` para pessoas sem telefone.|
...
| Parâmetro        | Tipo     | Obrigatório | Comparação | Descrição                                                                    |
| ---------------- | -------- | :---------: | ---------- | ---------------------------------------------------------------------------- |
| `entity`         | string   |     Sim     | exato      | Tipo da entidade vinculada. Valores aceitos: `people`, `companies`, `deals`. |
| `id`             | UUID     |     Sim     | exato      | UUID da entidade informada em `entity`.                                      |
| `created_after`  | ISO 8601 |     Não     | `>=`       | Atividades criadas a partir desta data.                                      |
| `created_before` | ISO 8601 |     Não     | `<=`       | Atividades criadas até esta data.                                            |

<Note>
  `entity` e `id` são sempre obrigatórios em conjunto. Requisições sem eles retornam `422`.
</Note>
...
# Atividades de uma pessoa criadas em 2026
