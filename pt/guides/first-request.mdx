...
Envie uma requisição de registro com o nome do app, e-mail de contato, redirect URIs e os escopos necessários. Este endpoint é público e não requer autenticação.
...
Use o `access_token` para fazer sua primeira chamada real à API:
...
O objeto `meta` informa o total de registros e se há mais páginas. Veja [Paginação](/pt/guides/pagination) para detalhes sobre como navegar em grandes conjuntos de resultados.

## O que explorar a seguir

<CardGroup cols={2}>
  <Card title="Paginação" icon="bars-staggered" href="/pt/guides/pagination">
    Navegue em grandes conjuntos de resultados com `page`, `per_page`, `order_by` e `order`.
  </Card>
  <Card title="Filtros" icon="filter" href="/pt/guides/filtering">
    Filtre por texto, intervalo de data, tags e campos adicionais.
  </Card>
