...
| `avatar_url`    | string   | Não         | URL HTTPS do logo do seu app. A Vendaze faz o download da imagem, valida o formato, converte para JPEG (máx. 400x400px) e armazena. Formatos aceitos: JPEG, PNG, WebP, GIF, BMP, TIFF. SVG não é aceito. Máx. 2MB. |
...
Quando `avatar_url` é fornecido, a Vendaze faz o download da imagem do seu servidor, valida o formato pelo magic byte do arquivo (não pelo Content-Type declarado), converte para JPEG (máx. 400x400px) e armazena na infraestrutura da Vendaze. O `avatar_url` na resposta é sempre uma URL `https://storage.vendaze.com`.
